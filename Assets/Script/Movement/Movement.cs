...
public class Movement : MonoBehaviour
{
    ...
    public float IceAcceleration = 10f; // Kecepatan akselerasi saat masuk ke es
    private Vector2 slopeTangent; // Arah tangen slope untuk sliding
    private bool wasRightwardSlope = false; // Track apakah slope sebelumnya rightward
    ...
    public void Move(InputAction.CallbackContext context)
    {
        ...
        // Abaikan input kiri saat di slope atau movement lock
        if ((OnIceSlope && wasRightwardSlope || MovementLocked) && SideMove < 0)
            SideMove = 0;
    }
    ...
    public void Jump(InputAction.CallbackContext context)
    {
        ...
        if (context.performed)
        {
            if (IsGrounded || OnIceSlope)
            {
                ...
                if (OnIceSlope && wasRightwardSlope && jumpDirection < 0)
                    jumpDirection = 0; // Abaikan loncat kiri di slope
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.CompareTag("Ice"))
        {
            ...
            // Track apakah ini rightward slope untuk lock logic
            wasRightwardSlope = (slopeTangent.x > 0);

            Debug.Log("On Ice (Collision), Slope Tangent: " + slopeTangent + ", Normal: " + contactNormal + ", Rightward: " + wasRightwardSlope);
        }
    }
    ...
    private void OnCollisionExit2D(Collision2D collision)
    {
        if (collision.collider.CompareTag("Ice"))
        {
            OnIceSlope = false;
            Debug.Log("Exit Ice (Collision)");
            // Hanya lock left movement jika slope sebelumnya rightward
            if (wasRightwardSlope)
            {
                StartCoroutine(LockLeftMovement(2f));
            }
        }
